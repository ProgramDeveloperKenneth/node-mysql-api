...
async function sendAlreadyRegisteredEmail(email: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `<p>Use <code>/account/forgot-password</code> to reset your password.</p>`;
    }
...
async function sendPasswordResetEmail(account: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `<p>Click to reset password (valid 1 day):</p><p><a href="${resetUrl}">${resetUrl}</a></p>`;
    } else {
